...
class TestCampaignDelete(APITestCase):
    ...
    def setUp(self):
        try:
            ...
            self.delete_url = reverse("campaign-detail", kwargs={"uid": self.campaign_uid})
        except Exception as e:
            self.fail(f"Erro inesperado no setup: {str(e)}")
            
    def test_delete_campaign_invalid_uid(self):
        """
        Testa a exclusão de uma campanha com um UID inválido.
        """
        delete_url = reverse("campaign-detail", kwargs={"uid": "847b4a59-5ca6-4335-bc97-b2a07e1eddc7"})
        response = self.client.delete(delete_url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertIn("detail", response.data)
        self.assertEqual(
            response.data["detail"],
            "Nenhuma campanha corresponde à consulta fornecida."
        )

    def test_delete_campaign(self):
        """
        Testa a exclusão de uma campanha.
        """
